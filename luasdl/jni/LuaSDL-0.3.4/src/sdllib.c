...
#include "SDL_net.h"
#endif
// http://sourceforge.net/p/kompozer/svn/149/tree/trunk/mozilla/xpcom/io/SpecialSystemDirectory.cpp?format=raw
...
static int _SDL_RWFromFP(lua_State* L)
{
    ...
    autoclose = luaL_checkinteger(L, 2);
    /* call */
    ret = SDL_RWFromFP(*pf, autoclose);
    /* result */
...
static int get_SDL_Rect_array(lua_State* L)
{
...
#endif
    {
		...
		int index;
        SDL_Rect** list;
		list = (SDL_Rect**)tolua_tousertype(L, 1, 0);
...
static int _SDL_GetKeyState(lua_State* L)
{
...
#endif
    {
        ...
        void* list = (void*)SDL_GetKeyboardState(&numkeys);
            ...
            while (c) {
                ...
                if (lua_isnil(L, 4)) {
                    ...
                    if (!tolua_isusertype(L, 4, "SDL_Rect", 0, &tolua_err))
                        break;
                    src = (SDL_Rect*)tolua_tousertype(L, 4, 0);
...
TOLUA_API int luaopen_LuaSDL (lua_State* L, struct LuaSDL_StartInfo *info)
{
    ...
    tolua_beginmodule(L,"SDL");
        ...
        /* user experience */
        tolua_sconstant(L, "LuaSDL_DirSep", info->dir_sep);
        tolua_sconstant(L, "LuaSDL_DirCwd", info->dir_cwd);
        tolua_sconstant(L, "LuaSDL_DirExe", info->dir_exe);
        tolua_sconstant(L, "LuaSDL_DirApp", info->dir_app);
        tolua_sconstant(L, "LuaSDL_AppName", info->appname);
        tolua_sconstant(L, "LuaSDL_NotFound", info->notfound);
